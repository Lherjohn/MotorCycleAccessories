...
<div class="container d-flex my-5 align-items-center justify-content-center" style="height: auto; font-size: .8rem">
    <div class="shadow p-4 rounded-3" style="width:350px; height: auto;">
        <?php if (isset($_GET['editProfile'])) {
            ...
            $editProfile = $conn->prepare("SELECT * FROM users WHERE userID = ?");
            $editProfile->execute([$userID]);
            foreach ($editProfile as $profile)   { ?>
                <form method="post" action="process.php">
                    <div class="mb-2">
                        <label for="exampleInputEmail1" class="form-label">First Name</label>
                        <input type="hidden" class="form-control" id="exampleInputEmail1" name="userID" value="<?= $profile['userID'] ?>">
                        <input type="text" class="form-control" id="exampleInputEmail1" name="fName" value="<?= $profile['userFirstName'] ?>">
                    </div>
                    <div class="mb-2">
                        <label for="exampleInputEmail2" class="form-label">Last Name</label>
                        <input type="text" class="form-control" id="exampleInputEmail2" name="lName" value="<?= $profile['userLastName'] ?>">
                    </div>
                    <div class="mb-2">
                        <label for="exampleInputEmail3" class="form-label">Email address</label>
                        <input type="email" class="form-control" id="exampleInputEmail3" aria-describedby="emailHelp" name="email" value="<?= $profile['userEmail'] ?>">
                    </div>
                    <div class="mb-2">
